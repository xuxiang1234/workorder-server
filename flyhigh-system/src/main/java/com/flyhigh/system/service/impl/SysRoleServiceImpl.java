...
import com.flyhigh.common.annotation.DataScope;
...
import com.flyhigh.system.domain.SysRoleMenu;
import com.flyhigh.system.domain.SysUserRole;
import com.flyhigh.system.mapper.*;
...
public class SysRoleServiceImpl implements ISysRoleService {
    ...
    public SysRole selectRoleById(Long roleId) {
        ...
        sysRole.setMenuIds(menuMapper.selectMenuListByRoleId(roleId, sysRole.isMenuCheckStrictly()));
        return sysRole;
    }
    ...
    public int insertAuthUsers(Long roleId, Long[] userIds) {
        if (null == roleId || ArrayUtils.isEmpty(userIds)) {
            return Constants.AJAX_RESULT_ERROR;
        }
        List<Long> userIdList = userRoleMapper.selectUserRoleByRoleId(roleId);

        // 新增用户与角色管理
        List<SysUserRole> list = new ArrayList<>();
        // 过滤已存在的用户ID
        Arrays.stream(userIds)
                .filter(item -> !userIdList.contains(item))
                .forEach(userId -> {
        ...
        return CollectionUtil.isEmpty(list) ? Constants.AJAX_RESULT_SUCCESS : userRoleMapper.batchUserRole(list);
    }
